...
st.title("📘 Claude & GPT 기반 차량 카탈로그 + 가격표 통합 RAG 질문응답")

ROOT_DIR = "C:/_knudata/Project_12/hyundaicar_info"
VECTORSTORE_DIR = "C:/_knudata/Project_12/vector_db"
...
if selected_model:
    ...
    if not os.path.exists(catalog_path):
        ...
        if question:
            ...
            st.subheader("🔍 기본 모델 응답 (PDF 참조 ❌)")
            col1, col2 = st.columns(2)
            with col1:
                st.markdown("💬 **GPT-4o 응답 (단독)**")
                st.info(gpt_direct_answer)
            with col2:
                st.markdown("💬 **Claude 응답 (단독)**")
                st.info(claude_direct_answer)

            st.markdown("### 📄 RAG 기반 응답 (PDF 참조 ✅)")
            col3, col4 = st.columns(2)
            with col3:
                st.markdown("🧠 **GPT RAG 응답**")
