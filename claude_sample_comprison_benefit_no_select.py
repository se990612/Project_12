...
# 차종 선택 안할 경우 전체 경로 탐색
# 전체 PDF 경로 수집 + VectorStore 통합
def get_all_pdf_paths(root_dir: str):
    pdf_paths = []
...
def load_global_vectorstore():
    all_pdfs = get_all_pdf_paths(ROOT_DIR)
    splitter = RecursiveCharacterTextSplitter(chunk_size=1000, chunk_overlap=200)
    all_docs = []
...
# ✅ 차량 미선택 시 전체 문서 기반 질문 UI 표시
if len(selected_models) == 0:
    st.subheader("📂 전체 현대차 문서 기반 질문")
    global_question = st.text_input("❓ 궁금한 점을 입력하세요 (예: 현대차 SUV 종류 알려줘)")

    if global_question:
        with st.spinner("Claude 전체 문서 기반 응답 생성 중..."):
            global_db = load_global_vectorstore()
            response = answer_with_claude(global_db, global_question)
